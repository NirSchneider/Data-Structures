...
public class CuckooHashing {
    ...
    public ArrayList<String> stash; //List of items that couldn't find a place

    private Stack<Triple> st;
        ...
        while (true) {
            ...
            for (int count = 0; count <= MAXTRIES; count++) {
                for (int i = 0; i < numHashFunctions; i++) {
                    ...
                    if (array[pos] == null) {
                        insert = new Triple(pos, null, this.kickGroup);
                        st.push(insert);
                        array[pos] = x;
                        currentSize++;
                        return true;
                ...
                insert = new Triple(kick_pos, tmp, this.kickGroup);
                array[kick_pos] = x;
                x = tmp;
                st.push(insert);
            }
            ...
            this.stash.add(x);
            insert = new Triple(-1, null, kickGroup);
            st.push(insert);
            return true;
        }
    }
    ...
    public void undo() {
        while (!st.isEmpty() && this.kickGroup == this.st.peek().getKickGroup()) //while in the same kick_group
        {
            Triple undo = st.pop();
            int ind = undo.getIndex();
            String prev = undo.getPreValue();
            if (ind != -1) // not from the stash
                this.array[ind] = prev;
            else
                this.stash.remove(stash.size()-1); // the last string always inserted at the end of the list
        }
        this.kickGroup--;
    }
    ...
    public boolean remove(String x) {
        ...
        currentSize--;
        st.clear(); // empty the stack
        kickGroup = 0; // reset the count of the kick_group
        return true;
    }
